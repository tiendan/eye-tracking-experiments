 ...
 * a jspsych plugin for browser compatibility checks for the eye-tracking experiments
 *
 * Onur Ferhat
 *
 */
  ...
  plugin.trial = function(display_element, trial) {
    var load_audio = function() {
      ...
      if(audio_context !== null){
        ...
        audio_source.buffer = jsPsych.pluginAPI.getAudioBuffer(trial.test_audio);
        audio_source.connect(audio_context.destination);
        audio_source.loop = true;
      } else {
        audio_buffer = jsPsych.pluginAPI.getAudioBuffer(trial.test_audio);
        audio_buffer.currentTime = 0;
        audio_buffer.loop = true;
      }
    }
    load_audio();
    ...
    if(is_mobile || ! is_supported || webgl_ctx == undefined) {
